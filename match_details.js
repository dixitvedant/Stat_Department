...
// NEW: Capture filter context to display in URL and fix navigation
const currentYear = params.get("year");
const currentTournament = params.get("tournament");

/**
 * SECTION: URL SYNC
 * This ensures that the address bar displays all three parameters.
 * It also handles the case where 'id' might be passed as 'matchId'.
 */
if (matchId) {
    const newUrl = new URL(window.location);
    newUrl.searchParams.set('id', matchId);
    // Only set if they exist and are not "all"
    if (currentYear && currentYear !== 'all') newUrl.searchParams.set('year', currentYear);
    if (currentTournament && currentTournament !== 'all') newUrl.searchParams.set('tournament', currentTournament);
    window.history.replaceState({}, '', newUrl);
}

// NEW: Update Back Button to remember filters
const backBtn = document.querySelector(".back-btn");
if (backBtn && currentYear && currentTournament) {
    backBtn.href = `match_wise.html?year=${encodeURIComponent(currentYear)}&tournament=${encodeURIComponent(currentTournament)}`;
}
...
function fillListWithStats(listId, idArray, statsContainerId) {
    ...
    idArray.forEach(playerId => {
        ...
        const contextSuffix = (currentYear && currentTournament) ? `&year=${encodeURIComponent(currentYear)}&tournament=${encodeURIComponent(currentTournament)}` : "";
        li.innerHTML = `<a href="playerstep1.html?search=${encodeURIComponent(playerId)}${contextSuffix}" class="roster-link">${displayName}</a>`;
        ul.appendChild(li);
    });
